      ...
      <section className="rounded-md bg-white p-6 shadow-soft">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-steel">产品会保存到本地 SQLite 数据库。</p>
          </div>
          {form.id && (
            <button
              type="button"
        ...
        {loading ? (
          ...
          <div className="mt-6 overflow-hidden rounded-md border border-ink/10">
            {filteredProducts.map((product) => (
              <article
                key={product.id}
                className="grid gap-4 border-b border-ink/10 p-4 last:border-b-0 lg:grid-cols-[1fr_150px_150px]"
              >
                ...
                <div className="text-sm text-steel">
                  <div className="font-medium text-ink">英文名称</div>
                  <div className="mt-2 line-clamp-2">{product.nameEn || "-"}</div>
                </div>
